/*
 * accel-mma84 driver
 * http://cylonjs.com
 *
 * Copyright (c) 2013-2014 The Hybrid Group
 * Licensed under the Apache 2.0 license.
*/

'use strict';

var Cylon = require('cylon');
var tessel = require('tessel');
var cameravc0706 = require('camera-vc0706');

var CameraVc0706 = module.exports = function () {
  CameraVc0706.__super__.constructor.apply(this, arguments);
  this.module = cameravc0706.use(tessel.port(this.connection.port));
  this.proxyMethods(this.commands, this.module, this);
};

Cylon.Utils.subclass(CameraVc0706, Cylon.Driver);

CameraVc0706.prototype.commands = [
  "resolutions","disable","setCompression","setResolution","takePicture",
];

CameraVc0706.prototype.start = function(callback) {
  var self = this;
  var args = arguments;

  self.module.on('ready', function () {
    self.defineEvent({ eventName: 'ready', source: self.module, target: self.device });
    self.defineEvent({ eventName: 'error', source: self.module, target: self.device });
    self.defineEvent({ eventName: 'compression', source: self.module, target: self.device });
    self.defineEvent({ eventName: 'picture', source: self.module, target: self.device });
    self.defineEvent({ eventName: 'resolution', source: self.module, target: self.device });
    
    CameraVc0706.__super__.start.apply(self, args);
    self.device.emit('start');
  });
};